export default class OrgChart {
  ...
  _createNode(nodeData, level) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (nodeData.hasOwnProperty('employees')) {
        nodeDiv.setAttribute('style', 'width: 100%');
      }
      ...
      nodeDiv.innerHTML = `<div class="title">${nodeData[opts.nodeTitle]}</div> `;
      nodeDiv.innerHTML += `<hr>`
      // let employeeDiv = level === 0 ? `<div class="flex d-flex flex-row" style="align-items: center; justify-content: center;">` : `<div>`;
      let employeeDiv = `<div class="flex d-flex ${level === 0 ? 'flex-row' : 'flex-column'}" style="align-items: center; justify-content: center;">`;
      if (nodeData.employees.length <= 0) {
        employeeDiv += `<div style="width: 200px">&nbsp;</div>`
      }
      for (let employee in nodeData.employees) {
        employeeDiv +=
